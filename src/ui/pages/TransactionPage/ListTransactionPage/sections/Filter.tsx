...
import { getListTransaction, setFilterTransaction } from "@/libs/redux/features/transaction.slice";
...
function Filter() {
  const { filter, status } = useAppSelector((state) => state.transactionSlice);
  const dispatch: AppDispatch = useDispatch();
  const [search, setSearch] = useState<string>(filter.search ?? "");
  const handleRefresh = () => {
    setSearch("");
    dispatch(setFilterTransaction(initialStateFilter));
