...
import { getListReturnTransaction, setFilterReturnTransaction } from "@/libs/redux/features/returnTransaction.slice";
...
function Filter() {
  const { filter, status } = useAppSelector((state) => state.returnTransactionSlice);
  const dispatch: AppDispatch = useDispatch();
  const [search, setSearch] = useState<string>(filter.search ?? "");
  const handleRefresh = () => {
    setSearch("");
    dispatch(setFilterReturnTransaction(initialStateFilter));
